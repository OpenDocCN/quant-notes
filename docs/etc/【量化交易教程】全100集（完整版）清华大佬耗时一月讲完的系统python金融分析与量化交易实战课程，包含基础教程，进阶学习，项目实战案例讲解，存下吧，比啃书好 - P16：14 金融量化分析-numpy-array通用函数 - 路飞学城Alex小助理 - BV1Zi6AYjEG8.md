# 【量化交易教程】全100集（完整版）清华大佬耗时一月讲完的系统python金融分析与量化交易实战课程，包含基础教程，进阶学习，项目实战案例讲解，存下吧，比啃书好 - P16：14 金融量化分析-numpy-array通用函数 - 路飞学城Alex小助理 - BV1Zi6AYjEG8
...
看第一个位置是true，其他的位置都是false哦，所以NAN这个值特别重要，到后边咱们学pandas这个库的时候，会知道NAN这个值是作为缺失值出现的啊，就比如说你一张数据表，你可能有几个是缺失的。
