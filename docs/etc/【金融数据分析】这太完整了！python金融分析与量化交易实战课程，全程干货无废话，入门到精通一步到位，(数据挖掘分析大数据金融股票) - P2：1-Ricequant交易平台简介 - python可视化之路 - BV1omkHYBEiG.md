# 【金融数据分析】这太完整了！python金融分析与量化交易实战课程，全程干货无废话，入门到精通一步到位，(数据挖掘分析大数据金融股票) - P2：1-Ricequant交易平台简介 - python可视化之路 - BV1omkHYBEiG
...
就是这些策略诶，大概是怎么去写，然后呢这样这里有一个哎这个名字啊。
...
在这里咱们说完了，哎，我们的第一块，第一块呢一个INIT函数啊，相当于就是一开始的时候诶，它会被先被执行的。
