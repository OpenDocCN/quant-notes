# 比刷剧还爽！2024最新Python金融分析与量化交易实战教程！三小时入门到进阶！就怕你不学！（AI人工智能丨数据分析丨数据挖掘丨机器学习实战丨深度学习丨编程） - P5：04-4-时间序列重采样操作 - AI算法-漆漆 - BV1py1vYfErs
...
然后这块一个STD这块也是点min一下，然后这里是点STD一下，就行了，然后指定完之后。
