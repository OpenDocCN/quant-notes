# 【比刷剧还爽！】这太完整了！python金融分析与量化交易实战课程，全程干货无废话 入门到精通一步到位！（数据挖掘分析／大数据／可视化／投资／金融／股票／算法） - P3：3.2-时间序列分析 - 李宏毅大模型_ - BV1JM4m1k7Jn
...
因为我们现在是可以指定日期的，他就要在这个日期当中啊进行一个选择，你看这里他会在我日期当中啊进行一个选择，选择时传说什么呢，选择出来一个start date，这是我的一个起始日期。
...
在这里你看就是在我data frame当中啊，我指定的一个date，它要干什么，它要大于等于起始时间，小于等于终止时间，这样呢就反应回去了，这个啊就是我们怎么样去取数据，根据我的一个起始时间和终止时间。

在数据当中啊进行了一个切分，只选你就是在这个时间片段之内的数据，指定colors colors啊，咱先指定多点一会儿，背部要画很多条线，反正咱们先指定出来，然后呢在这里就是去你看这块写了for循环啊。
